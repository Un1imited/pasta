...
- 🎨 **五套主题皮肤**（夜青 / 纯墨 / 宣纸 / 轻霜·真透明玻璃 / 随色·跟随系统强调色），全部通过 WCAG AA 对比度实测
...
从 [Releases](https://github.com/Un1imited/pasta/releases) 下载 `Pasta-1.9.0.zip`，解压拖到 `/Applications`。
...
| 切换 剪贴板 / 常用 | `⌘←` / `⌘→` |
| 粘贴选中项 | `⏎`（或双击） |
| 预览选中项 | `空格`（搜索框为空、或刚用方向键/标签导航后；再按或 `esc` 关闭） |
| 仅复制（不粘贴、不关面板） | `⌘C` |
| 直达粘贴第 N 条 | `⌘1` … `⌘9` |
| 纯文本粘贴 | `⌥⏎` |
| 加入 / 移出常用 | `⌘P` |
